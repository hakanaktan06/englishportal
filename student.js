...
window.submitWritingTask = async function() {
    ...
    // PATRONUN API KEY'İNİ ÇEKİYORUZ
    const { data: godProfile } = await supabaseClient.from('profiles').select('openai_key').eq('role', 'god').single();
    if (!godProfile || !godProfile.openai_key) {
        showToast("Sistem hatası: Öğretmenine haber ver.", "error");
        btn.innerHTML = originalText; btn.disabled = false; return;
    ...
    try {
        ...
        // Sonucu DB'ye açıklama (description) olarak yazıp ödevi Tamamlandı yapıyoruz
        const finalDesc = `[ÖĞRENCİ METNİ]\n${text}\n\n[YAPAY ZEKA DEĞERLENDİRMESİ]\n${aiFeedback}`;
        
        await supabaseClient.from('homeworks').update({ status: 'Tamamlandı', description: finalDesc }).eq('id', hwId);
        
        // XP Ver
        const { data: prof } = await supabaseClient.from('profiles').select('xp').eq('id', currentStudentId).single();
        await supabaseClient.from('profiles').update({ xp: (prof.xp || 0) + 100 }).eq('id', currentStudentId);

        showToast("Harika! Görev tamamlandı, XP kazanıldı.", "success");
        document.getElementById('writingTaskModal').classList.add('hidden');
        initStudentPortal(); // Sayfayı yenile
    } catch (err) {
        showToast('Bağlantı hatası, tekrar dene.', 'error');
    }
    btn.innerHTML = originalText; btn.disabled = false;
