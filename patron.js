...
// ==========================================
let allTeachersData = []; // Verileri hafızada tutalım ki mail atarken lazım olur

async function fetchSystemData() {
    // SADECE RLS KAPATILDIĞI İÇİN ÇALIŞIR
    const { data: teachers, error: tErr } = await supabaseClient.from('profiles').select('id, full_name, is_premium, created_at').eq('role', 'teacher').order('created_at', { ascending: false });
    ...
    if(tErr || !teachers) { tbody.innerHTML = '<tr><td colspan="4" class="p-6 text-center text-rose-500">Veri çekilemedi!</td></tr>'; return; }
    ...
    if (teachers.length === 0) {
        tbody.innerHTML = '<tr><td colspan="4" class="p-10 text-center text-slate-500 font-medium">Sistemde henüz kayıtlı öğretmen yok.</td></tr>';
        return;
    }
    ...
    teachers.forEach(teacher => {
        ...
        const actionBtn = isPremium
            ? `<button onclick="togglePremium('${teacher.id}', false)" class="text-[10px] md:text-xs bg-slate-800 hover:bg-rose-500/20 text-slate-400 hover:text-rose-400 border border-slate-700 hover:border-rose-500/30 px-3 py-2 rounded-xl font-bold transition">Yetkiyi Al</button>`
            : `<button onclick="togglePremium('${teacher.id}', true, '${teacher.full_name.replace(/'/g, "\\'")}')" class="text-[10px] md:text-xs bg-amber-500 hover:bg-amber-400 text-black px-3 py-2 rounded-xl font-black shadow-[0_0_15px_rgba(245,158,11,0.3)] transition transform active:scale-95 flex items-center gap-1 ml-auto">PREMİUM YAP <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg></button>`;
        ...
        tbody.innerHTML += `
            <tr class="hover:bg-slate-800/50 transition border-b border-slate-800/50">
                ...
                <td class="p-3 md:p-4 text-right flex items-center justify-end gap-2">
                    ${actionBtn}
                    <button onclick="deleteTeacher('${teacher.id}', '${teacher.full_name.replace(/'/g, "\\'")}')" class="bg-slate-800 hover:bg-rose-500 text-slate-500 hover:text-white p-2 rounded-xl transition border border-slate-700 hover:border-rose-500" title="Öğretmeni Sistemden Sil">
...
// ==========================================
// PREMİUM ONAY VE BİLDİRİM MEKANİZMASI
// ==========================================
window.togglePremium = async function(teacherId, makePremium, teacherName = '') {
    ...
    const onay = await customConfirm(actionText, makePremium ? "Evet, Premium Yap" : "Evet, İptal Et");
    if (!onay) return;
